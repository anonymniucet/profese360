/* ═══════════════════════════════════════════════════════════════
   Test scoringu — ověřuje, že výpočet SCORE odpovídá závazným
   tabulkám z MASTER SPECIFICATION V2.0 (kap. 5.3).

   Testuje přímo js/config.js a js/engine.js, tedy ten samý kód,
   který běží v prohlížeči — nic se nekopíruje ani negeneruje.

   Spuštění:  npm test        (nebo: node tests/score-test.js)
   ═══════════════════════════════════════════════════════════════ */
'use strict';

const { PROFESSIONS } = require('../js/config.js');
...
for (const key of Object.keys(PROFESSIONS)) {
  ...
  const at   = fn => Object.fromEntries(p.questions.map(q => [q.id, fn(q)]));
  ...
  /* ── Verzování výsledku (kap. 11) ── */
  chk(rMid.diagVersion === p.diagVersion && rMid.scoringVersion === p.scoringVersion,
      'výsledek nese verzi diagnostiky i scoringu');
  ...
  console.log('  otázek: ' + p.questions.length + ' · rozložení: ' +
              p.areas.map(a => a.key + '=' + p.questions.filter(q => q.area === a.key).length).join(' '));
}
